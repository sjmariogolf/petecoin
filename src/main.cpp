...
#if defined(NDEBUG)
# error "Petecoin cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Petecoin Signed Message:\n";
...
//TODO: this isn't identical to petecoin reference client.
...
{
    int64_t nSubsidy = 999 * COIN;

    std::string cseed_str = prevHash.ToString().substr(7,7);
    const char* cseed = cseed_str.c_str();
    long seed = hex2long(cseed);
    int rand = generateMTRandom(seed, 999);
    int rand1 = 0;

    if(nHeight < 10000 && !Params().SimplifiedRewards())
    {
    ...
    else if(nHeight > 20000 && nHeight < 30000)
    {
        ...
        cseed = cseed_str.c_str();
        seed = hex2long(cseed);
        rand1 = generateMTRandom(seed, 2999);
        nSubsidy = (1 + rand1) * COIN;
    }
    else if(nHeight > 30000 && nHeight < 40000)
    {
        ...
        cseed = cseed_str.c_str();
        seed = hex2long(cseed);
        rand1 = generateMTRandom(seed, 3999);
        nSubsidy = (1 + rand1) * COIN;
    }
    else if(nHeight > 40000 && nHeight < 50000)
    {
        ...
        cseed = cseed_str.c_str();
        seed = hex2long(cseed);
        rand1 = generateMTRandom(seed, 4999);
        nSubsidy = (1 + rand1) * COIN;
    }
    ...
    else if(nHeight > 60000 && nHeight < 70000)
    {
        ...
        cseed = cseed_str.c_str();
        seed = hex2long(cseed);
        rand1 = generateMTRandom(seed, 6999);
        nSubsidy = (1 + rand1) * COIN;
    }
    else if(nHeight > 70000 && nHeight < 80000)
    {
        ...
        cseed = cseed_str.c_str();
        seed = hex2long(cseed);
        rand1 = generateMTRandom(seed, 7999);
        nSubsidy = (1 + rand1) * COIN;
    }
    else if(nHeight > 80000 && nHeight < 90000)
    {
        ...
        cseed = cseed_str.c_str();
        seed = hex2long(cseed);
        rand1 = generateMTRandom(seed, 8999);
        nSubsidy = (1 + rand1) * COIN;
    }
    else
    {
...
static const int64_t nTargetTimespan = 1 * 60 * 60; // Petecoin: every 4 hours
static const int64_t nTargetTimespanNEW = 60 ; // Petecoin: every 1 minute
static const int64_t nTargetSpacing = 60; // Petecoin: 1 minute
...
unsigned int GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Petecoin: This fixes an issue where a 51% attack can change difficulty at will.
...
void ThreadScriptCheck() {
    RenameThread("petecoin-scriptch");
...
bool ConnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    // BIP16 was always active in Petecoin
...
{
    // Petecoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
