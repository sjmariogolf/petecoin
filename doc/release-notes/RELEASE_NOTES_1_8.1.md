...
Petecoin Core 1.8.1 is primarily a bugfix release, bringing Petecoin Core in
line with Bitcoin 0.9.3. Petecoin Core 1.8.1 also adds in support for printing
...
* Disable Transifex based translation process (not used by Petecoin Core)
* Add checkpoints around AuxPoW switchover block (371,337)
* Disable connections from pre-1.8 Petecoin Core clients
* Use HTTPS with petechain.info
...
There is a known issue with quantities in JSON used by the RPC interface (https://github.com/petecoin/petecoin/issues/134).
...
This only affects values significantly below 1 Pete, however merchants may wish to round to
the nearest Pete on all transactions for now.
