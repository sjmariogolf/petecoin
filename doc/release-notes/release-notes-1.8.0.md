...
Petecoin Core 1.8 introduces AuxPoW from block 371,337. AuxPoW is a technology
which enables miners to submit work done while mining other coins, as work
on the Petecoin block chain. Petecoin Core 1.8 also enables payment protocol
support for Petecoin (note that Petecoin's implementation of payment protocol is
...
as soon as possible afterwards in order to be able to continue to use Petecoin.
Users remaining on the previous client **will not** be able to receive or spend
Petecoin (with the exception of with other nodes who have not upgraded).
...
use "petecoind" you must run it the first time with the "-reindex" command line
option.
...
AuxPoW benefits Petecoin in two ways; firstly, the effective hashrate of the coin is
...
multipools from switching between Petecoin and other Scrypt coins.
...
In comparison to the AuxPoW implementation in United Scrypt Coin, Petecoin uses a
different coinbase transaction input script format which includes block height in
compliance with BIP0034.

## Payment Protocol

Petecoin Core 1.8 adds payment protocol support, which is used to streamline payments
being made to merchants.

This protocol is defined in DIP0070-DIP0072, based on the BIP standards with the same
assigned numbers. DIP standards can be found at https://github.com/petecoin/dips/ .
The key differencs between the Bitcoin and Petecoin payment protocols are that
the Petecoin payment request uses a "genesis" field containing the hash of the network's
...
(typically 1 Pete, or around $0.00012 per transaction).
...
of Petecoin compared to Bitcoin. This should lead to less false positives.

A problem in parsing mangled petecoin: URIs under Windows has been resolved, and as
part of this work network detection for petecoin: URIs is now more robust.
