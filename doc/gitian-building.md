...
*Setup instructions for a gitian build of Petecoin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Petecoin
...
- [Building Petecoin](#building-petecoin)
...
Clone the git repositories for petecoin and gitian,

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/petecoin/petecoin
```
...
In [doc/release-process.md](release-process.md) in the petecoin repository under 'Fetch and build inputs'.
...
To build Petecoin (for Linux and/or Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the petecoin repository.

Output from `gbuild` will look something like

    Initialized empty Git repository in /home/debian/gitian-builder/inputs/petecoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/petecoin/petecoin
...
For example:
```bash
URL=https://github.com/langerhans/petecoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit petecoin=${COMMIT} --url petecoin=${URL} ../petecoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit petecoin=${COMMIT} --url petecoin=${URL} ../petecoin/contrib/gitian-descriptors/gitian-win.yml
```
...
```bash
    gpg --detach-sign ${VERSION}/${SIGNER}/petecoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/petecoin-build.assert
```
...
[petecoin/gitian.sigs](https://github.com/petecoin/gitian.sigs/) repository, or
